...
gen_save_file_path = lambda suffix: os.path.join(script_dir, "flop_cnn_training_note_%d.%s" % (timestamp, suffix))
...
training_data1_path = root + "/learning_data/data/win_rate/flop/50000-data-1000-simulation-2-players-win-rate-data.csv"
training_data2_path = root + "/learning_data/data/win_rate/flop/100000-data-1000-simulation-2-players-win-rate-data.csv"
...
history = model.fit(train_x, train_y, batch_size=128, nb_epoch=5000, validation_split=0.1, verbose=2)
...
for card1, card2, card3, card4, card5, expected in test_case:
    ...
    print("HOLE = [%s, %s], COMMUNITY = [%s, %s, %s] => win_rate = { prediction=%f, expected=%f }" % tuple(list(map(str, hole)) + list(map(str, community)) + [y , expected]))
